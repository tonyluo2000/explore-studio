    ...
    *,
    ...
    engine_npcs = tuple(
        ClassroomTrailNPC(
            ...
            respond_to_toggle=(
                None
                if entry.character.respond_to_toggle is None
                else ClassroomTrailNPCConditionalResponse(
                    toggle_qualified_id=(
                        f"{entry.provenance.package_id}:"
                        f"{entry.character.respond_to_toggle.object_id}"
                    ),
            ...
            respond_to_either_toggle=(
                None
                if entry.character.respond_to_either_toggle is None
                else ClassroomTrailNPCEitherToggleResponse(
                    toggle_qualified_ids=(
                        f"{entry.provenance.package_id}:{entry.character.respond_to_either_toggle.object_ids[0]}",
                        f"{entry.provenance.package_id}:{entry.character.respond_to_either_toggle.object_ids[1]}",
                    ),
                    when_both_off=entry.character.respond_to_either_toggle.when_both_off,
                    when_either_on=entry.character.respond_to_either_toggle.when_either_on,
                )
            ),
            respond_to_counter=(
                None
                if entry.character.respond_to_counter is None
                else ClassroomTrailNPCCounterResponse(
                    counter_qualified_id=(
                        f"{entry.provenance.package_id}:"
                        f"{entry.character.respond_to_counter.object_id}"
                    ),
                    when_below_goal=entry.character.respond_to_counter.when_below_goal,
