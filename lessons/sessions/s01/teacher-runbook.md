...
|---:|---|---|
| 0:00–0:05 | Show the local trail. Ask, “What would an explorer write down here?” | Names one sensory or story detail. |
| 0:05–0:12 | Model a string literal and `print`. Before running, ask which visible things should count toward M01 completion. | Predicts that world objects—not the player or guide—fill the visited count. |
| 0:12–0:24 | Open `student/starter.py`. Students personalize exactly three observations and run the file after each change. | Three readable printed lines. |
| 0:24–0:37 | Launch M01. Students move near and interact with every classroom object, comparing the count with their prediction. | Both objects visited and the trail reports completion. |
| 0:37–0:42 | Give the quoting bug below. Students predict the cause, repair it, and rerun. | Explains that an opening quote needs a matching closing quote. |
| 0:42–0:45 | Ask for one observation and one test result. Guide the Git close. | Reviews status and diff, then makes one descriptive commit. |
...
The student reads the diff aloud before committing.
